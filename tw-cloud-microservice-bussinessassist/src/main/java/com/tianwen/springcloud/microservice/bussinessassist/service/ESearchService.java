...
public class ESearchService{
    ...
    public String getSqlQueryResult(String query) throws Exception{
        ...
        StringEntity entity = new StringEntity(jsonQuery.toJSONString(), ContentType.APPLICATION_JSON);
    ...
    public JSONObject getQueryStringNull( String field){
        JSONObject query = new JSONObject();
    ...
    public void fixQueryTree(QueryTree queryTree) throws Exception{
        ...
        if (queryTree.getQueryCondition("onelabel") != null) {
            ...
            if (noCondFlag == false) {
                if (!CollectionUtils.isEmpty(onelabelids)) {
                    onelabelids.add("");
        ...
        if (queryTree.getQueryCondition("twolabel") != null) {
            ...
            if (noCondFlag == false) {
                if (!CollectionUtils.isEmpty(twolabelids)) {
                    twolabelids.add("");
        ...
        if (queryTree.getQueryCondition("threelabel") != null) {
            ...
            if (noCondFlag == false) {
                if (!CollectionUtils.isEmpty(threelabelids)) {
                    threelabelids.add("");
        ...
        if (queryTree.getQueryCondition("themeids") != null)
        {
            ...
            if (!value.isEmpty()) {
                ...
                resources.add("");
                resources.add("");
                queryTree.addCondition(new QueryCondition("contentids", QueryCondition.Prepender.AND, QueryCondition.Operator.EQUAL, resources));
            }
        }
    }
    ...
    public Response notFoundServerError() {
        return new Response(IStateCode.HTTP_404, IBussinessMicroConstants.ES_SERVER_ERROR);
    }
}
